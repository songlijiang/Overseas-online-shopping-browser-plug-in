function localStartX(){
	...
	return function(plugin_content){
		...
		plugin_content.site_id = 20;
		plugin_content.site_name = 'gap';
		...
		plugin_content.get_img = function(){
			var  goods_img = "http://www.gap.com"+$("#product_image").attr("src");
			return goods_img;
		}
		...
		plugin_content.get_price = function(){
			var prodPrice ="$0.1";
			if($("span#priceText").length >0)
			{
				prodPrice = $("span#priceText").text();
			}
			else if($("#quickLookColorSwatches").length >0)
			{
				proPrice =$("#quickLookColorSwatches").text();
			}
		...
		plugin_content.get_category = function(){
			var prodcategory = "";
			$("ul.gpnavigation").find("a").each(function(){
