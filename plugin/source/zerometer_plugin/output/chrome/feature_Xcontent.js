function localStartX(){
	...
	return function(plugin_content){
		...
		plugin_content.site_id = 17;
		plugin_content.site_name = 'feature';
		...
		plugin_content.get_img = function(){
			var  goods_img =  $('#product-photos').find(".thumbnail.desktop-2").eq(0).attr("src");
			return goods_img;
		}
		...
		plugin_content.get_features = function(){
			...
			function getSizesNames(){
			  var value = "";
			  var hasAttr =false;
			  if($(".swatch.clearfix").find("label").length >0)
			  	{
			  		hasAttr = true;
